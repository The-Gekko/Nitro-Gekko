...
enum acer_wmi_predator_v4_oc {
 ...
 /* Acer Nitro 17 AN17-51 (Spacia_RTH). Mismo esquema que los AN16-4x:
    Gaming-WMI v4 y teclado RGB de 4 zonas (ficha oficial de Acer:
    "Backlit keyboard: Yes (4 zones RGB)"). Sin esta entrada el modelo no se
    detecta y el grupo sysfs four_zoned_kb no llega a crearse. */
...
//  }
 ...
 static ssize_t predator_fan_speed_store(struct device *dev,
     ...
     /* memcpy en vez de strncpy: en kernel 7.2 strncpy ya no esta declarado.
        Es sustitucion directa porque len < sizeof(input) y el NUL se pone
        explicitamente justo debajo. */
 ...
 static ssize_t four_zoned_rgb_kb_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count) {
     ...
     /* memcpy: ver nota sobre strncpy en kernel 7.2 mas arriba. */
